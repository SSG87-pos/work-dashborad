...
- 2026-07-09 simplified dashboard UX branch: `codex/simplified-dashboard-ux` changes the user-facing first-screen direction for 40대 이상/non-specialist users. Browser review showed that adding a compact guidance strip still made the first screen feel more complex, so the corrected approach is to keep the original POSLAB entry screen, then show `오늘 브리핑`/today work first and hide board, filters, timeline, materials, mindmap, recurring work, and archive behind one quiet `전체 업무 보기` action. The duplicated four-card workflow summary (`내 진행 업무`, `3일 내 마감`, `지연 업무`, `완료 업무`) stays hidden even after opening the full workflow because it overlaps with the briefing. The default `나의 업무` first screen keeps search/compact AI/account/`업무 추가` visible, hides secondary top utilities such as density toggle and entry-return, and shows notifications only when unread. The visible briefing header stays minimal: no greeting/helper copy under `오늘 브리핑`, and no `자동 선별 n건` sticker. Team Members, account, admin people rows, and archive owner labels now use initials/name/role rather than visible profile emojis; the account modal no longer exposes profile emoji selection. The `전체 업무 보기` affordance is now a button only, without adjacent helper text explaining hidden board/filter/timeline controls. Top-level labels are simpler (`나의 업무`, `팀 업무`, `일정`, `업데이트`, `자료/보고`). This should stay frontend-only unless a later implementation explicitly expands scope. `DESIGN-apple.md` is reference material only: absorb restrained Apple-inspired operational details such as Action Blue, soft gray surfaces, thin hairlines, readable body sizing, and reduced decorative gradients/shadows, but do not replace the work dashboard with Apple-style product-gallery heroes or full-viewport marketing tiles.
- 2026-07-09 latest simplified-identity/helper verification: `check:simplified-ux`, `check:summary-filter`, `git diff --check`, production build, and escalated `curl -I http://127.0.0.1:5174/` passed after switching visible people identity surfaces from emoji to initials and removing the helper sentence beside `전체 업무 보기`. In-app Browser automatic navigation from the previous error page was blocked by Browser URL policy, so browser QA for the identity/helper follow-up still needs a later retry.
