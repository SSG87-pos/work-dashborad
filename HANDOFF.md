...
- 2026-07-09 simplified dashboard UX branch: `codex/simplified-dashboard-ux` changes the user-facing first-screen direction for 40대 이상/non-specialist users. Browser review showed that adding a compact guidance strip still made the first screen feel more complex, so the corrected approach is to keep the original POSLAB entry screen, then show `오늘 브리핑`/today work first and hide board, filters, timeline, materials, mindmap, recurring work, and archive behind one quiet `전체 업무 보기` action. The duplicated four-card workflow summary (`내 진행 업무`, `3일 내 마감`, `지연 업무`, `완료 업무`) stays hidden even after opening the full workflow because it overlaps with the briefing. The default `나의 업무` first screen keeps search/compact AI/account/`업무 추가` visible, hides secondary top utilities such as density toggle and entry-return, and shows notifications only when unread. The simplified shell now tones down decorative gradients/shadows on the title, AI/account controls, selected team rows, and common cards in favor of solid text, white/soft-gray surfaces, thin borders, and one restrained active accent. The visible briefing header stays minimal: no greeting/helper copy under `오늘 브리핑`, and no `자동 선별 n건` sticker. Empty `오늘 일정` and empty `업무 Note`/`팀 체크` side widgets are hidden on the default first screen so the briefing list can use the width, but they return when populated or when the full workflow is open. Default-screen briefing rows hide the round status emoji and tag chips so rows read as status, task title, and due date first. The team list label is `팀원`, and team rows show centered name/role only without emoji or initials; the top account button also removes the leading initials badge. Account/admin/archive owner identity outside these simplified shell controls can keep initials where useful for signed-in-person clarity. The account modal no longer exposes profile emoji selection. The `전체 업무 보기` affordance is now a button only, without adjacent helper text explaining hidden board/filter/timeline controls. The duplicated workspace team strip is hidden on the default first screen. Top-level labels are simpler (`나의 업무`, `팀 업무`, `일정`, `업데이트`, `자료/보고`). This should stay frontend-only unless a later implementation explicitly expands scope. `DESIGN-apple.md` is reference material only: absorb restrained Apple-inspired operational details such as Action Blue, soft gray surfaces, thin hairlines, readable body sizing, and reduced decorative gradients/shadows, but do not replace the work dashboard with Apple-style product-gallery heroes or full-viewport marketing tiles.
- 2026-07-09 latest simplified-identity/helper verification: `check:simplified-ux`, `check:summary-filter`, `git diff --check`, production build, and Browser QA on the active local demo server passed after toning down the simplified shell gradients/shadows, simplifying default briefing rows by hiding round status emoji/tag chips, hiding empty `오늘 일정`/`업무 Note` side widgets on the default first screen, centering team member name/role rows, removing team emoji/initial badges, removing the top account button initials badge, changing the label to `팀원`, removing the helper sentence beside `전체 업무 보기`, and hiding the duplicated workspace team strip on the default first screen. Browser style audit on the default first screen measured visible gradients reduced to 0, visible box-shadows reduced to 2, and the remaining strong blue fill limited to `업무 추가`.
