...
- Backend direction is now approved at the architecture level:
  ...
  - top insight cards can now use FastAPI `GET /api/v1/dashboard/insights` to show DB-computed counts/evidence for 진행, 3일 내 마감, 지연, 업데이트 정체, 흐름 미지정, and 완료 cohorts; local fallback still computes the same filters from the current task array.
  - Latest dashboard insight QA: backend pytest passed with `38 passed`; `check:api-store`, `check:summary-filter`, `git diff --check`, and production build passed. Browser QA on local fallback `http://127.0.0.1:5187/` verified six top insight cards, no horizontal overflow, quiet zero-count `흐름 미지정`, and `업데이트 정체` click producing the expected summary filter pill and narrowed board lane counts.
