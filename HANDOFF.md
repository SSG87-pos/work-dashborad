...
- LLM-Wiki backend Phase 1 is implemented on branch `codex/llm-wiki-architecture`:
  ...
  - Frontend API boundary: `src/apiStore.js` exposes `apiDashboardStore.ai.wiki.search/readPage/readSources/listDrafts/createDraft/createDraftFromDashboard/approveDraft/rejectDraft/recordError`, and `scripts/check-api-store.mjs` verifies those paths.
  - Implemented routes under `/api/v1/ai/wiki/...`: search, page read, link follow, source read, draft list/create/dashboard-task draft create, admin approval/rejection, and Error Book correction create.
  ...
  - Admin now has a first `Wiki 초안` review tab that lists pending drafts and lets admins approve/publish or reject them.
  ...
  - Remaining later phase: production image/file storage, automatic OCR/HERmes/OpenAI Vision extraction into the same attachment fields, OpenAI/HERmes/MCP tool calling around these backend routes, Highlights/workstream Wiki surfaces, and inline draft editing.
