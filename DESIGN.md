...
- Avoid nested-card buildup without making controls feel detached. When full workflow opens, tabs, filters, tags, status guide, and board/list/timeline content should read as one connected work surface. The workflow surface should be a quiet gray work mat, lane headers should not become white pill buttons, lane columns should stay subtly grounded, and task cards should be the primary white information objects. Briefing rows should use separators rather than row cards.
