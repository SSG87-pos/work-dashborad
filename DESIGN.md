...
- My Desk may include a compact `내 글` channel surface under the briefing that gathers the selected user's authored task posts. It should offer `내가 쓴 글` and `팀 공개 글` tabs without replacing the task-detail writing flow.
- Team Flow may include a compact `업무 채널` surface under the briefing that groups `팀 공개` task posts by `상위 업무흐름`. It should start as a low-height lookup/review surface, not a second full social feed beside the workflow board.
