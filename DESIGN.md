...
- Apple-inspired operational styling may be used only as a restraint layer: use `#0066cc` Action Blue for primary actions/selection, `#f5f5f7` for the quiet app canvas, `#ffffff` cards with `#e0e0e0` hairlines, pill search/actions where appropriate, and no decorative shadows on common dashboard chrome. Keep operational cards at the dashboard's compact 8px radius even though `DESIGN-apple.md` includes larger marketing-card radii. Do not apply Apple marketing-page patterns such as full-viewport product tiles, photography-first heroes, ultra-thin 12px nav, or negative letter spacing to this work dashboard.
