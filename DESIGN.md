...
- Avoid nested-card buildup without making controls feel detached. When full workflow opens, tabs, filters, tags, status guide, and board/list/timeline content should read as one connected work surface; lane columns can have a quiet gray surface, while task cards stay as the primary white information objects. Briefing rows should use separators rather than row cards.
