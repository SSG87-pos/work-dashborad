...
- In `업무자료 > 업무별 노트`, each task group may read like a quiet channel: the task title is the channel header, and note rows show author, date, source, type, title, preview text, and compact source actions. Keep it task-linked and evidence-oriented, not an open chat room.
- In `업무자료 > 업무별 노트`, the task-level action should be labeled `상세보기`, stay chip-sized beside the 업무 Note/log counts, and use a slightly clearer blue treatment than count chips. It should not use the larger shared secondary-button treatment inside the channel header.
- In `업무자료 > 업무별 노트`, do not show author initials/avatar chips in the message list. Use author name in metadata instead. Clicking the note title should open an in-app read dialog, not a new browser window.
- Images pasted into task-level `업무 Note` should read naturally as inline evidence thumbnails wherever those notes are reviewed. Clicking a thumbnail should open the existing larger image preview/lightbox. Keep pasted images attached to the note record rather than embedded as uncontrolled rich text inside the body.
