...
- [x] Implement simplified dashboard first-screen UX on `codex/simplified-dashboard-ux`.
  ...
  - Done: simplified top-level labels to `나의 업무`, `팀 업무`, `일정`, `업데이트`, and `자료/보고`; kept the original POSLAB entry/login screen; made entry/login start in `나의 업무` focus mode; removed the added four-button guidance strip from the default work screen; hid board/filter/workflow controls until `전체 업무 보기`; removed the duplicated four-card workflow summary from the visible workflow; and hid secondary top utilities on the first screen while keeping search, account, and `업무 추가`.
  - Verified: `check:simplified-ux`, `check:summary-filter`, `git diff --check`, production build, and Browser QA on fallback Vite `http://127.0.0.1:5174/`. Browser QA confirmed the original POSLAB entry screen is still intact, the default `나의 업무` screen shows only account/`업무 추가` in the top action area when there are no unread notifications, AI/density/entry-return controls reappear after `전체 업무 보기`, the four summary cards stay hidden, and 390px phone width has no page-level horizontal overflow. Existing Three/lanyard deprecation warnings remain unrelated to this change.
