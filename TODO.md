...
- [x] Implement simplified dashboard first-screen UX on `codex/simplified-dashboard-ux`.
  ...
  - Done: simplified top-level labels to `나의 업무`, `팀 업무`, `일정`, `업데이트`, and `자료/보고`; kept the original POSLAB entry/login screen; made entry/login start in `나의 업무` focus mode; removed the added four-button guidance strip from the default work screen; removed the `전체 업무 보기` reveal button and shows the workflow tabs, compact tag/filter line, and board directly under `오늘 브리핑`; removed the duplicated four-card workflow summary from the visible workflow; removed the top `넓게/기본` density toggle from the simplified shell; toned down shell gradients/shadows on AI/account controls, selected team rows, sidebar decoration, and common cards while preserving the top title gradient; removed the visible briefing greeting/helper line plus `자동 선별 n건` sticker; kept the `오늘 브리핑` row structure recognizable; shows empty `오늘 일정`/`업무 Note`/`팀 체크` side widgets as quiet placeholders on desktop/tablet for consistency; changed visible 팀원 rows to centered name/role only without emoji or initials; removed the top account button leading initials badge; kept account/admin identity initials elsewhere only where they still help identify the signed-in person; removed the account-modal profile emoji picker; changed the team list label from `Team Members` to `팀원`; tightened the tag/filter line; made workflow tabs visibly selectable; rebuilt the workflow surface from a dark gray slab into a bright white work panel without a broad blue gradient; restored restrained status color through lane tints/card edge accents/status chips; changed board/list status labels to plain `검토/계획/진행/완료/보류`; and balanced board cards as title, compact priority/context, at most one context badge, quiet inline tags, and a `+n` tag summary without repeating the lane status inside each card.
  - Verified: latest pass ran `check:simplified-ux`, `check:summary-filter`, `git diff --check`, production build, and Browser QA on the active local demo server after neutralizing the sidebar brand divider/sublabel, restoring the top title gradient, keeping default briefing row icon/tag structure, toning down shell shadows on secondary controls/cards, restoring empty `오늘 일정`/`업무 Note` placeholders on the simple first screen, centering team member name/role rows, removing the top account button initials badge, and keeping team badges/workspace team strip removed. Browser QA confirmed the title has the approved gradient, default briefing rows show 2 visible icons and 2 visible tag chips in the demo data, sidebar brand divider is a neutral 1px line, and AI/account selected-team controls keep no decorative background gradients. 2026-07-09 Apple component follow-up now treats `DESIGN-apple.md` as a full component grammar: very light neutral sidebar against `#fbfcff` parchment canvas, `#0066cc` primary/selection color, 44px primary/secondary/search pills, 38px pearl utility capsules, white hairline cards, pearl segmented/filter controls, and no card/control shadow. Workflow surface repair follow-up fixed the over-flattened state by reconnecting workflow tabs, tag/filter controls, status guide, and board/list/timeline content into one quiet work surface; the board now uses a clean white panel without a broad blue gradient, lane headers stay transparent instead of becoming white pill buttons, lanes use a subtly grounded status tint, and task cards remain the primary white objects. Sidebar follow-up removed the selected nav/team boxed-card look: active nav now uses transparent background, blue text/icon, and a 2px blue indicator with no shadow; selected team member rows use text color/weight only with no background box or left rail. The sidebar surface now uses only a light neutral gray so the page does not become a flat field of white, while avoiding a dull dark-gray panel. Previous browser QA on the same simplified branch confirmed compact AI is visible on the default `나의 업무` first screen, clicking it opens the `Work-Hub AI` floating window, density/home/empty notification controls remain hidden in the simple state, and 390px phone width keeps AI plus `업무 추가` with no page-level horizontal overflow.
  - Latest board polish verification: workflow tabs, tag/filter strip, and board now measure as one continuous surface with 0px visual gaps; board comparison lanes measured equal height; board cards no longer render repeated status chips; card tags render as transparent inline metadata; switching `보드 → 리스트 → 보드` works; desktop and 390px mobile checks showed no page-level horizontal overflow.
  - Latest shell polish verification: 1021px Browser QA confirmed the top title/action row now reads as a frosted neutral-gray app toolbar instead of a rounded card or large colored floating block across dashboard tabs, AI/account/`업무 추가` buttons use compact role-based widths rather than oversized equal pills, AI keeps a light Action Blue treatment so it remains discoverable, the toolbar uses a very light neutral header fill rather than a blue-tinted band, the search field sits below the toolbar as a quiet command field, the work canvas is near-white instead of a heavy gray/blue field, the sidebar uses a very light neutral gray surface for hierarchy without making the app dull, the workflow board panel uses a clean white surface without a broad blue gradient, the workflow tabs/tag/filter/status-guide rows also use white surfaces, the `오늘 브리핑` label has no direct divider underneath, empty `오늘 일정` and `업무 Note` placeholders render, high-priority board chips use the restored red priority color, `스팟` chips and spot-card edges use teal again, card height was reduced after removing doubled padding, board lanes hug visible card content instead of leaving large bottom whitespace, empty `0건` lanes stay collapsed as header-only lanes, and 390px mobile still has no page-level horizontal overflow.
...
- [x] Polish entry/dashboard AI assistant and clean the top bar.
  ...
  - Done: changed the assistant from a single latest-answer card into a chat-thread surface with explicit modes for `질문`, `업무 등록`, `업무 Note`, and `상태 변경`. Write-like requests create confirmation cards first. `업무 등록` opens the existing 업무 추가 form from a reviewed draft; `업무 Note` finds related task candidates and lets the user save as either a task update log or `업무자료` post; `상태 변경` finds related task candidates and updates status only after confirmation. Meeting/minutes/material/document-style notes default to `업무자료`, while ordinary comments/progress notes default to logs, with a manual override before saving. Follow-up replies such as `응` or `좋아 저장해줘` execute the latest pending card in the same thread.
  ...
  - Done: dashboard top bar now includes a compact `🤖 AI` button that opens a persistent floating AI widget. The widget can be moved by dragging its header and stays constrained inside the viewport. The compact AI button remains visible on phone layouts beside `업무 추가`.
...
- [x] Document future personal notification inbox path.
  ...
  - Product decision: keep Phase 1 simple and high-signal. Show each person only their own must-check alerts: newly assigned work, overdue owned work, and logs/notes/risk notes left by someone else on their work. Do not turn every status change or due-soon signal into an alarm.
  - Implemented: Phase 1 now has a FastAPI/PostgreSQL `notifications` migration/model/API, task assignment/update/post/overdue notification creation, `src/apiStore.js` notification methods, top bell unread badge, notification panel, unread/all filters, single-read, all-read, and task detail click-through.
  - Verified: `/private/tmp` Python 3.12 venv `python -m pytest` passed 43 tests including notification owner-only/read-state coverage; bundled-Node production build passed.
  - Guardrail: company PostgreSQL apply/smoke, dismiss/hide UI, Teams push, real-time push, mobile push, due-soon notification rows, and user-configurable alert rules remain later scope.
...
- [x] Retune standard desktop layout density.
  ...
  - 2026-06-27 follow-up: Notebook L 100 percent is now treated as the desktop baseline, with Notebook L 81 percent only as a density reference. Responsive compaction moved below about 980px so 1024px landscape tablet keeps the desktop-style sidebar/topbar composition, while 768px portrait can still compact.
  - 2026-06-27 portrait-tablet follow-up: 640-980px now keeps the full dashboard context but uses initials for team/account avatars, a two-row top toolbar, briefing side widgets below the briefing list, and tighter non-overlapping board lanes. POSLAB entry hides the lanyard/card visual at these widths and below, then centers the remaining entry panel.
  - 2026-06-27 insight-card follow-up: clicking a top insight card now applies the summary filter, opens the first matching task in the workflow detail panel, scrolls naturally to that panel, and closes the hover preview by blurring the clicked card. Hover previews remain available on mouse hover and keyboard focus-visible.
  - 2026-06-27 portrait-tablet dashboard follow-up: at tablet-portrait widths, `Strategy Work Hub` and the dashboard actions share the title row while search stays on the next full-width row; the sidebar stays as a 96px icon rail, team members move to a one-line workspace strip, the account/profile button shows the name only, summary cards switch to 2x2, and briefing rows keep category/content/due columns aligned across single-task and multi-task rows.
  - 2026-06-27 workflow readability follow-up: list-view 업무 제목 now wraps and shows the full title instead of truncating with ellipsis, and expanded tablet-portrait board lanes span the full workflow width even when they have fewer than five cards.
  - 2026-06-27 board expansion follow-up: board card titles also show the full 업무명, and Notebook/tablet board expansion now distinguishes comparison mode from focus mode. Multiple open lanes stay as side-by-side status columns; a single open lane with at least three cards expands to the full board width.
  - 2026-06-27 board/detail correction: when a workflow task detail is open, the board returns to compact columns instead of keeping focus-mode wide expansion, preventing the selected lane from forcing a wide horizontal scroll beside the detail panel.
  - 2026-06-27 briefing responsive correction: at narrow desktop/notebook widths, keep the briefing task list full-width and place `오늘 일정` plus `팀 체크`/`업무 Note` below it so important task titles are not squeezed by side widgets.
  - 2026-06-27 phone layout correction: iPhone-class widths now hide the duplicate workspace team strip, keep the phone topbar as title/search/full-width task creation, and lock briefing rows to icon/content/due columns so due labels stay aligned on the row.
  - 2026-06-27 phone alignment follow-up: phone team member controls now use equal short-name chips like portrait tablet, insight card number/label stacks are centered, and all briefing due labels share the same right rail. At extra-narrow phone widths and the narrowest portrait-tablet boundary, briefing tag chips are hidden so task titles keep priority over metadata.
