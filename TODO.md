...
- [x] Implement simplified dashboard first-screen UX on `codex/simplified-dashboard-ux`.
  ...
  - Done: simplified top-level labels to `오늘`, `팀 업무`, `일정`, `업데이트`, and `자료/보고`; made entry/login start in `오늘` focus mode; removed the added four-button guidance strip from the default work screen; hid board/filter/workflow controls until `전체 업무 보기`; and applied restrained Apple-inspired operational surface tokens without replacing the dashboard with a marketing layout.
  - Verified: `check:simplified-ux`, `check:summary-filter`, `git diff --check`, production build, and browser QA on fallback Vite `http://127.0.0.1:5174/`. Browser QA confirmed the simplified entry screen hides lanyard/entry AI, `오늘` opens with no guidance strip/insight cards/board/filter controls, `전체 업무 보기` restores the full workflow, and 390px phone width has no page-level horizontal overflow.
