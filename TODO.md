...
- [x] Implement simplified dashboard first-screen UX on `codex/simplified-dashboard-ux`.
  ...
  - Done: simplified top-level labels to `나의 업무`, `팀 업무`, `일정`, `업데이트`, and `자료/보고`; kept the original POSLAB entry/login screen; made entry/login start in `나의 업무` focus mode; removed the added four-button guidance strip from the default work screen; hid board/filter/workflow controls until `전체 업무 보기`; removed the duplicated four-card workflow summary from the visible workflow; hid secondary top utilities on the first screen while keeping search, account, and `업무 추가`; and removed the visible briefing greeting/helper line plus `자동 선별 n건` sticker.
  - Verified: `check:simplified-ux`, `check:summary-filter`, `git diff --check`, production build, and Browser QA on fallback Vite `http://127.0.0.1:5174/`. Browser QA confirmed the briefing header shows no `자동 선별 n건` sticker and no greeting/helper sentence under `오늘 브리핑` on desktop and 390px phone width, while the first screen still has `전체 업무 보기` and no page-level horizontal overflow. Existing Three/lanyard deprecation warnings remain unrelated to this change.
