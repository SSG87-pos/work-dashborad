...
- [x] Implement simplified dashboard first-screen UX on `codex/simplified-dashboard-ux`.
  ...
  - Done: simplified top-level labels to `나의 업무`, `팀 업무`, `일정`, `업데이트`, and `자료/보고`; kept the original POSLAB entry/login screen; made entry/login start in `나의 업무` focus mode; removed the added four-button guidance strip from the default work screen; hid board/filter/workflow controls until `전체 업무 보기`; removed the duplicated four-card workflow summary from the visible workflow; hid secondary top utilities on the first screen while keeping search, compact AI, account, and `업무 추가`; and removed the visible briefing greeting/helper line plus `자동 선별 n건` sticker.
  - Verified: `check:simplified-ux`, `check:summary-filter`, `git diff --check`, production build, and Browser QA on fallback Vite `http://127.0.0.1:5174/`. Browser QA confirmed compact AI is visible on the default `나의 업무` first screen, clicking it opens the `Work-Hub AI` floating window, density/home/empty notification controls remain hidden in the simple state, and 390px phone width keeps AI plus `업무 추가` with no page-level horizontal overflow. Existing Three/lanyard deprecation warnings remain unrelated to this change.
