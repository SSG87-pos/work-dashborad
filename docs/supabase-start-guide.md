...
- `docs/company-c3-supabase-docker-runbook.md` explains the current C3 Docker image/multi-container Supabase operating path, including `release/company-supabase-c3`, no-sudo `$ADMIN` command handling, C3 stack requirements, Supabase `.env`, migration `001...022`, and smoke tests for 업무 인박스/팀 체크.
